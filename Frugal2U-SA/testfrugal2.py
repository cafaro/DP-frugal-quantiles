...
import math
import os
from testfrugal2_params import *
#import pandas as pd
import glob
...
def test_on_quantiles(outdir):
    test_count = 1
    for q in q_values:
        outputdir = outdir + "/test_stream_quantile_" + str(test_count)
        ...
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            ...
            proc.append(sbc.run([exec_name, "-n", ni_default, "-q" , q, "-d", d_default, "-e", eps_default, "-k", k_default, "-f", outputfile, "-s", str(seed)]))
...
def test_on_stream_lengths(outdir):
    ...
    for n in ni_values:
        ...
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            ...
            proc.append(sbc.run([exec_name, "-n", n, "-q" , q_default, "-d", d_default, "-e", eps_default, "-k", k_default, "-f", outputfile, "-s", str(seed)]))
            seed_count = seed_count + 1
            print_to_stderr("#")
...
def test_on_distributions(outdir):
    ...
    for d in d_values:
        ...
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            ...
            proc.append(sbc.run([exec_name, "-n", ni_default, "-q" , q_default, "-d", d, "-e", eps_default, "-k", k_default, "-f", outputfile, "-s", str(seed)]))
            seed_count = seed_count + 1
            print_to_stderr("#")
...
def test_on_epsilon(outdir):
    ...
    for eps in eps_values:
        outputdir = outdir + "/test_stream_epsilon_" + str(test_count)
        os.makedirs(outputdir, exist_ok=True)
        ...
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            ...
            proc.append(sbc.run([exec_name, "-n", ni_default, "-q" , q_default, "-d", d_default, "-e", eps, "-k", k_default, "-f", outputfile, "-s", str(seed)]))
            seed_count = seed_count + 1
            print_to_stderr("#")
...
def test_on_k(outdir):
    test_count = 1
    for k in k_values:
        outputdir = outdir + "/test_stream_chunks_" + str(test_count)
        os.makedirs(outputdir, exist_ok=True)
        print_to_stderr('Test chunks = ' + k + '\n')

        seed_count = 1
        proc = []
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            outputfile = outputdir + "/test_stream_chunks_" + str(test_count) + "_" + str(seed_count) + ".csv"
            proc.append(sbc.run([exec_name, "-n", ni_default, "-q" , q_default, "-d", d_default, "-e", eps_default, "-k", k, "-f", outputfile, "-s", str(seed)]))
            seed_count = seed_count + 1
            print_to_stderr("#")
