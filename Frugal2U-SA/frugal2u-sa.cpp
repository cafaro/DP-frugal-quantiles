...
int main(int argc, char **argv)
{
  ...
  int *sign = NULL;
  int chunks = 4;
  double upper = INT_MAX;
  double lower = INT_MIN;
  double epsilon = 0.1;
  ...
  case 1:
    ...
    {
      items[i] = normaldistribution(generator) * PREC;
    }
  ...
  case 2:
    for (long i = 0; i < len; i++)
    {
      items[i] = cauchydistribution(generator) * PREC;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
    {
  ...
  case 3:
    for (long i = 0; i < len; i++)
    {
      items[i] = uniformrealdistribution(generator) * PREC;
      ;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
  ...
  case 4:
    for (long i = 0; i < len; i++)
    {
      items[i] = exponentialdistribution(generator) * PREC;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
    {
  ...
  case 5:
    for (long i = 0; i < len; i++)
    {
      items[i] = chisquareddistribution(generator) * PREC;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
    {
  ...
  case 6:
    for (long i = 0; i < len; i++)
    {
      items[i] = gammadistribution(generator) * PREC;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
    {
  ...
  case 7:
    for (long i = 0; i < len; i++)
    {
      items[i] = lognormaldistribution(generator) * PREC;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
    {
  ...
  case 8:
    for (long i = 0; i < len; i++)
    {
      items[i] = extremevaluedistribution(generator) * PREC;
    }
    diststr = (char *)calloc(16, sizeof(char));
    if (!diststr)
    {
  ...
  upper = (double)(*max_element(vec.begin(), vec.end()) / PREC);
  lower = (double)(*min_element(vec.begin(), vec.end()) / PREC);
  // upper = items[0];
  // lower = items[0];
  // for (int i = 0; i < len; i++)
  // {
  ...
  // double relative_error = fabs((eq / PREC - (double)true_quantile / PREC)) / fabs((double)true_quantile / PREC);
  ...
  boost::random::laplace_distribution<double> laplace(0.0, (upper - lower) / (chunks * epsilon));
  boost::variate_generator<boost::random::mt19937 &, boost::random::laplace_distribution<double>> laplace_gen(rng, laplace);
  double laplace_noise = laplace_gen();
  double dp_laplace_estimated_quantile = (eq / PREC) + laplace_noise;
  ...
  double dp_rel_err = fabs((dp_laplace_estimated_quantile - (double)(true_quantile / PREC)) / (double)(true_quantile / PREC));
  double dp_norm_abs_err = fabs((dp_laplace_estimated_quantile - (double)(true_quantile / PREC)) / (upper - lower));

  fprintf(stdout, "the relative error/nae for the DP estimated quantile is: %.6f/%.6f\n", dp_rel_err, dp_norm_abs_err);
  ...
  if (file_output)
  {
    ...
    //<updates/s>, <epsilon>, <estimated sensitivity>, <chunks>, <laplace dp estimate>, <DP norm abs error>
    //<DP relative error>
    fprintf(fptr, "%ld, %.2f, %s, %.6f, %.6f, %ld, %.6f, %.6f, %.6f, %ld, %.6f, %.6f, %d, %.6f, %.6f, %.6f\n",
            len, quantile, diststr, param1, param2, seed,
            eq / PREC, (double)true_quantile / PREC,
            elapsed, lround(len / elapsed), epsilon, (upper - lower) / chunks, chunks, dp_laplace_estimated_quantile, dp_norm_abs_err, dp_rel_err);
    fclose(fptr);
    free(filename), filename = NULL;
  }
