...
int ldp_randomized_response(double q, double r, double x, std::mt19937 &gen1,
                            std::mt19937 &gen2)
{
    ...
    // }
    u = bernoulli_r(gen1);
    v = bernoulli_one_half(gen2);

    // if (bernoulli_one_half(gen2)) {
    //     v = 1;
    // } else {
    //     v = 0;
    // }
...
void usage(void)
{
    ...
    fprintf(
        stderr,
        "-n <number of items to be generated> default: 100 millions of items\n");
    ...
    fprintf(stderr, "-r <data filename>\n");
}

void read_data_from_file(char *filename, double **items, long *len)
{
    FILE *df = fopen(filename, "r");
    if (!df)
    {
        log(1, "Data file not found\n");
        exit(1);
    }
    double item;
    int count = 0;
    while (fscanf(df, " %lf", &item) != EOF)
        count++;

    *len = count;
    *items = (double *)calloc(count, sizeof(double));

    rewind(df);
    count = 0;
    while (fscanf(df, " %lf", &((*items)[count++])) != EOF)
        ;
}
...
int main(int argc, char **argv)
{
    ...
    double eps = 2.0; // privacy budger

    int opt;

    while ((opt = getopt(argc, argv, ":n:q:e:d:a:b:s:f:h:r:")) != -1)
    {
        switch (opt)
        {
        case 'n':
        ...
        case 'f':
            ...
            break;
        case 'r':
            datafile = (char *)calloc(strlen(optarg) + 1, sizeof(char));
            if (!datafile)
            {
                log(!file_output, "not enough memory\n");
                exit(1);
            }
            memcpy(datafile, optarg, strlen(optarg));
            file_input = true;
            break;
        case 'h':
            usage();
    ...
    if (file_input)
    {
        read_data_from_file(datafile, &items, &len);
    }
    else
    {
    ...
    std::mt19937 generator1(seed2);
    std::mt19937 generator2(seed3);
    if (!file_input)
    {
    ...
    // Begin algorithm kernel
    long n = 0;
    double r = std::tanh(eps / 2.0);

    for (long i = 0; i < len; i++)
    {
        n = n + 1;
        double stepsize = 2 / (std::pow((double)n, 0.51) + 100.0);

        double norm_item = (items[i] - smin) / range;

        int s = ldp_randomized_response(qn, r, norm_item, generator1, generator2);
        if (s == 1)
            qn = qn + ((1.0 - r + 2.0 * quantile * r) / 2.0) * stepsize;
        else
            qn = qn - ((1.0 + r - 2.0 * quantile * r) / 2.0) * stepsize;

        Qn = ((n - 1) * Qn + qn) / n;
    }
    // end algorithm kernel
    ...
    double estimated_quantile = Qn * range + smin;
    double abs_error = fabs(estimated_quantile - true_quantile);
    double norm_abs_error = abs_error / range;
    double relative_error =
        fabs((estimated_quantile - true_quantile) / true_quantile);

    log(!file_output, "Epsilon: %.2f\n", eps);
    log(!file_output, "r corresponding to epsilon: %.9f\n", r);
    ...
    if (file_output)
    {
        ...
        fprintf(fptr,
                ...
                "%.6f,%.6f,%ld\n",
                len, quantile, eps, diststr, param1, param2, seed, Qn,
